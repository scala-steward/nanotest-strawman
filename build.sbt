  ...
  .nativeSettings(
    ...
    libraryDependencies ++= {
      if (scalaBinaryVersion.value != "3") {
        Seq(
          compilerPlugin(
            "com.github.ghik" % "silencer-plugin" % "1.7.11" cross CrossVersion.full
          )
        )
      } else {
        Nil
    ...
    libraryDependencies += {
      if (scalaBinaryVersion.value == "3") {
        "com.github.ghik" % "silencer-lib_2.13.7" % "1.7.11" % Provided
      } else {
        "com.github.ghik" % "silencer-lib" % "1.7.11" % Provided cross CrossVersion.full // required for 0.3.9 support
      }
    },
